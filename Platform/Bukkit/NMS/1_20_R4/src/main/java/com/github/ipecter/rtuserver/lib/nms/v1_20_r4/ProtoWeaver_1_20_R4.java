...
public class ProtoWeaver_1_20_R4 implements BukkitProtoWeaver {
    ...
    public ProtoWeaver_1_20_R4(String folder) {
        info("ProtoWeaver_1_21_R1 initialized.");
        this.folder = folder;
        ProtoWeaver.PROTOCOL_LOADED.register(protocol -> {
            info("Protocol loaded.");
            ProtoLogger.setLogger(this);
            SSLContext.initKeystore(folder);
            SSLContext.genKeys();
            SSLContext.initContext();
            if (PaperLib.isPaper()) {
                info("Paper: " + GlobalConfiguration.get().proxies.velocity.secret);
